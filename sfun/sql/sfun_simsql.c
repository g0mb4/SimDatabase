#define S_FUNCTION_NAME  sfun_simsql
...
static void mdlInitializeSizes(SimStruct * S) {
    ...
    if (!mxIsNumeric(ssGetSFcnParam(S, 1)) ||
        ...
        ssSetErrorStatus(S, "Second parameter is the port, integer (eg. "
                            "3306).");
        return;
    }

    if (!mxIsChar(ssGetSFcnParam(S, 2))) {
        ssSetErrorStatus(S, "Third parameter is the username, string (eg. "
                            "'root').");
        return;
    }

    if (!mxIsChar(ssGetSFcnParam(S, 3))) {
        ssSetErrorStatus(S, "Fourth parameter is the password, string (eg. "
                            "'toor', '').");
        return;
    }

    if (!mxIsChar(ssGetSFcnParam(S, 4))) {
        ssSetErrorStatus(S, "Fifth parameter is the database, string (eg. "
                            "'sim_test').");
        return;
    }

    if (!mxIsChar(ssGetSFcnParam(S, 5))) {
        ssSetErrorStatus(S, "Sixth parameter is the tabe, string (eg. "
                            "'data').");
        return;
    }
    ...
    if (!mxIsNumeric(ssGetSFcnParam(S, 11)) ||
        mxGetNumberOfElements(ssGetSFcnParam(S, 11)) != 1) {
    ...
    if (mywrapper_init(addr, mxGetScalar(ssGetSFcnParam(S, 1)), // port
                       user, pass, db, table, start_day, start_time, end_day,
                       end_time, sensors) < 0) {
