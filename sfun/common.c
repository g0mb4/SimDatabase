...
void str_trim(char * out, const char * str, const char * trim) {
    unsigned int tlen = (unsigned int)strlen(trim);
    unsigned int i, j = 0;
    bool found;
...
time_t str_date_time_to_time(const char * date, const char * time) {
    ...
    sscanf(date, "%d-%d-%d", &tm.tm_year, &tm.tm_mon,
           &tm.tm_mday); // convert DATE
    sscanf(time, "%d:%d:%d", &tm.tm_hour, &tm.tm_min,
           &tm.tm_sec); // convert TIME
...
double interpolation_linear(double x0, double y0, double x1, double y1,
                            double x) {
